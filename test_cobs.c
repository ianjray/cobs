...
int main(void)
{
...

    {
        ...
        assert(-EFAULT  == cobs_decode_finish(NULL));
    ...
    {
        assert(-EFAULT == cobs_decode(NULL,    0, NULL, 0));
        assert(-EFAULT == cobs_decode(encoded, 0, NULL, 0));

        uint8_t m[] = { 0x01, 0x11, 0x22 };
        assert(-ENOSPC == cobs_decode(m, sizeof(m), unencoded, 1));
    }

    {
        ...
        unencoded_length = cobs_decode(encoded, (size_t)encoded_length, unencoded, sizeof(unencoded));
...

    {
        ...
        unencoded_length = cobs_decode(encoded, (size_t)encoded_length, unencoded, sizeof(m));
        assert(unencoded_length == sizeof(m));
        assert(memcmp(m, unencoded, sizeof(m)) == 0);
    }
